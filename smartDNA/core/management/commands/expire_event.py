...
def deleteVerification():
    ...
    for index in range(q.count()):
     ...
     print Verification.objects.filter(scan_time__lt=datetime.datetime.now()-datetime.timedelta(days=int(days))).exclude(status=1).count()," verification record deleted successfuly"
     Verification.objects.filter(scan_time__lt=datetime.datetime.now()-datetime.timedelta(days=int(days))).exclude(status=1).delete()
