...
def setAccessCounter():
  f = open(WORK_PATH+'access_count', 'r+')
  line= f.readline()
...
def getAddressElements(geo_location):
...
def getAddressComponents(lat,lng):
  address_components={'postal_code':'none','locality':'none','state':'none'}
  try:
    req_url='https://maps.googleapis.com/maps/api/geocode/json?latlng=%s,%s'%(lat,lng)
    request = Request(req_url)
    #print req_url
    f = urlopen(request)
    fr=f.read()
    results=json.loads(fr)
    result=results['results'][0]['address_components']
    for item in result:
      types=item["types"]
      if 'postal_code' in types:
        address_components['postal_code']=item['long_name']
      if 'locality' in types:
        address_components['locality']=item['long_name']
      if 'administrative_area_level_1' in types:
        address_components['state']=item['long_name']
  except:
    try:
      address=results['results'][0]['formatted_address']
      addr_list=address.split(',')
      print addr_list[-1],addr_list[-2],addr_list[-3]
      address_components['postal_code']=addr_list[-1]
      address_components['locality']=addr_list[-2]
      address_components['state']=addr_list[-3]
    except:
      print 'not found!'
...
#2017-05-16 08:23:33
def export_workbook(d1,d2,header_style=None,default_style=None,cell_style_map=None):
#def export_workbook(header_style=None,default_style=None,cell_style_map=None):
    from phisf.models import Verification
    #queryset=Verification.objects.all().exclude(status=1)
    # d1="2017-05-16 08:23:33"
    # d2="2017-05-16 18:23:33"
    queryset=Verification.objects.filter(scan_time__range=(d1, d2)).order_by('-scan_time').exclude(status=1)
    print "total records",queryset.count()
    columns = (
        'state',
        'city',
        'postal_code',
    ...
    for x, obj in enumerate(queryset, start=1):
      i=i+1
      lat,lng=str(getattr(obj, "geo_location")).split('|')
      addr_comp=getAddressComponents(lat,lng)
      print i, addr_comp
      for y, column in enumerate(columns):
       if str(column) == "state":
        try:
          value =str(addr_comp['state'])
        except:
          value='none'
       elif str(column) == "city":
        try:
          value =str(addr_comp['locality'])
        except:
          value='none'
       elif str(column) == "postal_code":
        try:
          value =str(addr_comp['postal_code'])
        except:
          value='none'
       elif str(column) == "counter":
        value='1'
       else:
