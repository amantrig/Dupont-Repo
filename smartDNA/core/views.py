...
from core.forms import AnomalyForm,LoginForm, CheckForm, FetchForm,ScanAnalysisForm,DateRangeForm,BatchReportForm,SettingsForm,UpdateProductDetailsForm
from core.models import Deployment
from django.conf import settings
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger
from core.model_utils import identify_noise,tamp_state
from core.utils import queryset_to_mediabook,export_media,export_media2,post_data_2sv,compress_image,getStatusCode,getAddress,sendSMS,setAccessCounter
      ...
      for records in json_data:
       ...
       if scanner_mode!="regular":
         ...
         if location['state']=="none":
          geo_location = '22.501261|73.473488'
          location ={'street':'Yashkamal Industrial Estate','locality':'Baska-Tajpura Road','city':'Halol','state':'Gujarat','postal_code':'389350','country':'India'}
       ...
       try:
        ...
        if status_code==100 and regCount>0:
         ...
         if scanner_mode=="regular" and status_code==1:
          ...
          r.save()
          #print 'registration',r.asset_code
         if scanner_mode=="regular" and status_code==2:
          v=Verification(**records)
...
def sms_fetch(request):
    ...
    if 'RGTR' in comments:
     ...
     scan_auth=sender
     bit_mask=record[11]
     productDetails=record[12]
     color_profile='nft'
     img_link="/media/documents/signature-not-found-4.png"
     try:
      company_name=record[13]
     except:
     ...
     from smartdna.models import Verification
     from smartdna.models import Registration
     ...
     if status_code==1 and Verification.objects.filter(asset_code__iexact=asset_code,status=1).count()>0:
      ...
      save_data.saveData(asset_code,scan_time,time_millis,credential,int(orrcredential), d1,d2,d3,h1,h2,h3,angle,status,status_code,geoLoc,location['street'],location['locality'],location['city'],location['state'],location['postal_code'],location['country'],operator,username,password,scan_auth,productDetails,bit_mask,color_profile,img_link,email_id,company_name)
      data='Success'
    else:
   ...
   if request.method == "POST":
     ...
     if form.is_valid(): # All validation rules pass
       ...
       if status=="Registered":
        model_name='Registration'
        geoLoc= '17.67505645751953|78.49069213867188'
        location ={'street':'none','locality':'Srinagar - Kanyakumari Hwy','city':'Hydrabad','state':'Telangana','postal_code':'501401','country':'India'}
...
def live_chart_data(request):
    ...
    try:
     Verification = get_model(dep_path, 'Verification')
     verification=Verification.objects.filter(status__in=['1','2','3','5','16','10'])
     ...
     eRows = verification.filter(status=16).count()
					...
					if sh.nrows<10002:
						for rx in range(1,sh.nrows):
							try:
								...
								obj.product_details=sh.row(rx)[1].value[:pd_max_length]
								obj.save()
								i=i+1;
