...
class VerificationManager(models.Manager):
    def get_queryset(self):
        return super(VerificationManager, self).get_queryset().filter(company_name='DuPont')
...
class Registration(models.Model):
    ...
    def release(self,*args,**kwargs):
        print "releasing xxxxxx the assets!"
        asset_code=self.asset_code
        print asset_code
        relcnt=0
        regcnt=0
        relcnt=Registration.objects.filter(asset_code__exact=asset_code,status=10).count()
        regcnt=Registration.objects.filter(asset_code__exact=asset_code,status=1).count()

        if (relcnt >0):
          v = Registration.objects.filter(asset_code__exact=asset_code,status=10)
          for x in v:
            #x.scan_time=datetime.datetime.fromtimestamp
            x.save()
        if (regcnt >0):
          v=Registration.objects.filter(asset_code__exact=asset_code,status=1)
          if (relcnt>0):
